...
Also add `TELEGRAM_BOT_POOL` to the launchd plist (`~/Library/LaunchAgents/com.sheep.plist`) in the `EnvironmentVariables` dict if using launchd.
...
# macOS:
launchctl unload ~/Library/LaunchAgents/com.sheep.plist
launchctl load ~/Library/LaunchAgents/com.sheep.plist
# Linux:
...
>
> Check logs: `tail -f logs/sheep.log | grep -i pool`
...
2. Check pool initialized: `grep "Pool bot" logs/sheep.log`
...
8. Rebuild: `npm run build && ./container/build.sh && launchctl unload ~/Library/LaunchAgents/com.sheep.plist && launchctl load ~/Library/LaunchAgents/com.sheep.plist` (macOS) or `npm run build && ./container/build.sh && systemctl --user restart sheep` (Linux)
