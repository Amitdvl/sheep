...
async function getEntityName(
  ...
  try {
    const entity = (await client.getEntity(peer as never)) as unknown as Record<
      string,
      unknown
    >;
    if (entity.className === 'User') {
      ...
      const name =
        parts.length > 0
          ? parts.join(' ')
          : (entity.username as string) || 'Unknown';
...
export function startTelegramUserClient(): void {
  ...
  const apiHash =
    process.env.TELEGRAM_API_HASH || envVars.TELEGRAM_API_HASH || '';
  const sessionStr =
    process.env.TELEGRAM_USER_SESSION || envVars.TELEGRAM_USER_SESSION || '';
  ...
  if (isNaN(apiId)) {
    logger.warn(
      { apiIdStr },
      'Telegram user client: TELEGRAM_API_ID is not a valid number',
    );
    return;
  }
  ...
  Promise.all([
    ...
    .then(([{ TelegramClient }, { StringSession }, { NewMessage }]) => {
      ...
      client
        .connect()
        .then(async () => {
          const me = (await client.getMe()) as unknown as Record<
            string,
            unknown
          >;
          const myName =
          ...
          client.addEventHandler(
            async (event: { message: Record<string, unknown> }) => {
              ...
              const timestamp = new Date(
                (msg.date as number) * 1000,
              ).toISOString();
              const isFromMe = Boolean(msg.out);

              // Get chat name and group status
              const { name: chatName, isGroup } = await getEntityName(
                client,
                peerId,
              );
              ...
              storeMessageDirect({
                ...
                sender: isFromMe
                  ? 'me'
                  : String(
                      (msg.fromId as Record<string, unknown>)?.userId ?? '',
                    ),
              ...
              logger.debug(
                { chatJid, chatName, isFromMe },
                'Telegram user message stored',
              );
            },
            new NewMessage({}),
          );
        ...
        .catch((err: Error) => {
          logger.error(
            { err: err.message },
            'Telegram user client failed to connect',
          );
        });
    })
    .catch((err: Error) => {
