...
<dict>
    ...
    <string>${projectRoot}/logs/sheep.log</string>
    <key>StandardErrorPath</key>
    <string>${projectRoot}/logs/sheep.error.log</string>
</dict>
</plist>`;
  ...
  try {
    ...
    serviceLoaded = output.includes('com.sheep');
  } catch {
...
/**
 * Kill any orphaned sheep node processes left from previous runs or debugging.
...
function killOrphanedProcesses(projectRoot: string): void {
  try {
    ...
    logger.info('Stopped any orphaned sheep processes');
  } catch {
...
function setupSystemd(
  ...
  if (runningAsRoot) {
    unitPath = '/etc/systemd/system/sheep.service';
    ...
    unitPath = path.join(unitDir, 'sheep.service');
    systemctlPrefix = 'systemctl --user';
  }

  const unit = `[Unit]
Description=Sheep Personal Assistant
...
StandardOutput=append:${projectRoot}/logs/sheep.log
StandardError=append:${projectRoot}/logs/sheep.error.log
...
WantedBy=${runningAsRoot ? 'multi-user.target' : 'default.target'}`;
  ...
  // Kill orphaned sheep processes to avoid channel connection conflicts
  ...
  try {
    execSync(`${systemctlPrefix} enable sheep`, { stdio: 'ignore' });
  } catch (err) {
    logger.error({ err }, 'systemctl enable failed');
  }

  try {
    execSync(`${systemctlPrefix} start sheep`, { stdio: 'ignore' });
  } catch (err) {
  ...
  let serviceLoaded = false;
  try {
    execSync(`${systemctlPrefix} is-active sheep`, { stdio: 'ignore' });
    serviceLoaded = true;
  } catch {
    // Not active
  }
...
function setupNohupFallback(
  ...
  const wrapperPath = path.join(projectRoot, 'start-sheep.sh');
  const pidFile = path.join(projectRoot, 'sheep.pid');

  const lines = [
    '#!/bin/bash',
    '# start-sheep.sh — Start Sheep without systemd',
    ...
    '    echo "Stopping existing Sheep (PID $OLD_PID)..."',
    ...
    `  >> ${JSON.stringify(projectRoot + '/logs/sheep.log')} \\`,
    `  2>> ${JSON.stringify(projectRoot + '/logs/sheep.error.log')} &`,
    '',
    `echo $! > ${JSON.stringify(pidFile)}`,
    'echo "Sheep started (PID $!)"',
    `echo "Logs: tail -f ${projectRoot}/logs/sheep.log"`,
  ];
