...
src/container-runner.ts               container/agent-runner/
    │                                      │
    │ spawns container                      │ runs Codex Agent SDK
...
|-----|----------|---------|
| **Main app logs** | `logs/sheep.log` | Host-side WhatsApp, routing, container spawning |
| **Main app errors** | `logs/sheep.error.log` | Host-side errors |
...
If sessions aren't being resumed (new session ID every time), or Codex exits with code 1 when resuming:

**Root cause:** The SDK looks for sessions at `$HOME/.Codex/projects/`. Inside the container, `HOME=/home/node`, so it looks at `/home/node/.Codex/projects/`.
...
echo "HOME=$HOME"
ls -la $HOME/.Codex/projects/ 2>&1 | head -5
'
```
...
docker run --rm --entrypoint /bin/bash \
  ...
  Codex -p "Say hello" --dangerously-skip-permissions --allowedTools ""
'
```
...
The agent-runner uses these Codex Agent SDK options:
...
**Important:** `allowDangerouslySkipPermissions: true` is required when using `permissionMode: 'bypassPermissions'`. Without it, Codex exits with code 1.
...
```bash
grep "Session initialized" logs/sheep.log | tail -5
...
SESSIONS=$(grep "Session initialized" logs/sheep.log 2>/dev/null | tail -5 | awk '{print $NF}' | sort -u | wc -l)
